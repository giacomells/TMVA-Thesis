...
void histCreation()
{
    ...
    TFile *output = new TFile("AnalysisBDT.root", "RECREATE");
    ...
    TH1F *histInvariantMassBDT = new TH1F("histInvariantMass", "Invariant Mass; m_{inv}(pK^{0}_{S})[GeV/#it{c}^{2}]; Entries", 60, 2.1, 2.43);
    ...
    for (Int_t i = 0; i < nevents; i++)
    {
        ...
        if (LcPt < 1.0 && LcPt > 0.0)
        {
            ...
            {
                histInvariantMassBDT->Fill(massLc2K0Sp);
            }
        }
    }
...
{
    // FILE CREATION
    TFile *output = new TFile("AnalysisBDT.root");
    // HISTOGRAM CREATION
    TH1F *histInvariantMassBDT = (TH1F *)output->Get("histInvariantMass"); //, "Invariant Mass; m_{inv}(pK^{0}_{S})[GeV/#it{c}^{2}]; Entries", 60, 2.1, 2.43);
    TH1F *histDifference = (TH1F *)histInvariantMassBDT->Clone("histDifference");
    ...
    // BACKGROUND FITTING
    histInvariantMassBDT->Fit("backgroundFit", "Q", "", 2.1, 2.43);
    histInvariantMassBDT->DrawCopy();

    // LOOP FOR CREATION OF HISTDIFFERENCE
    for (int i = 1; i <= histInvariantMassBDT->GetNbinsX(); ++i)
    {
        // Troviamo il centro del bin
        double binCenter = histInvariantMassBDT->GetBinCenter(i);
        // Valutiamo il fit in questo punto
        double fitValue = backgroundFit->Eval(binCenter);
        // Otteniamo il contenuto del bin originale
        double binContent = histInvariantMassBDT->GetBinContent(i);
    ...
    for (int i = 1; i <= histDifference->GetNbinsX(); ++i)
    {
        ...
        double newError = currentError * 12;
    ...
    canvas2->SaveAs("histDifferenceBDT.root", "RECREATE"); // Salva come immagine PNG
